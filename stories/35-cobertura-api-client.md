...
**43% linhas / 30% funcs**, rumo ao **mínimo de 80% linhas** (política da rodada). É o ponto único de
HTTP + socket; bug aqui vaza pra todo o produto.
...
- `packages/api-client/src/client.ts` (hoje ~42%) ≥ **80% linhas**: `request` (GET/POST/PATCH/DELETE),
  ...
  follow/summary).
- `packages/api-client/src/socket.ts` (hoje ~0%) ≥ 80%: conexão com token, `subscribe:store`/eventos,
  reconexão, cleanup/`disconnect`.
- `packages/api-client/src/token-store.ts` coberto (get/set/clear; fallback de storage).
- **Agregado do workspace ≥ 80% linhas.** Sem `--passWithNoTests`; gate `perFile`/ratchet da story 19
  respeitado (piso sobe pra 80).
