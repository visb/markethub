...
/**
 * Story 42: AuthProvider do app picker. Mocka o ApiClient (construtor → instância
 * fake controlável), o createRealtimeClient (sem socket real) e o expo-secure-store
 * (SecureTokenStore). Cobre carga de sessão (papel certo/errado/sem sessão), login
 * (sucesso e gate de papel `picker`) e logout. Sem rede.
 */
...
const mockRealtime = { connect: jest.fn(), disconnect: jest.fn(), on: jest.fn() };
...
jest.mock("@markethub/api-client", () => {
  ...
  return {
    ...
    createRealtimeClient: jest.fn(() => mockRealtime),
  };
});
...
const pickerUser = { id: "u1", name: "Sep", email: "s@x.com", roles: ["picker"] };
...
describe("AuthProvider — carga de sessão", () => {
  it("sessão válida com papel picker popula o usuário", async () => {
    mockClient.me.mockResolvedValue(pickerUser);
    const { captured, unmount } = renderProvider();
    await flush();
    expect(captured.current?.user?.name).toBe("Sep");
...
describe("AuthProvider — login", () => {
  it("login com papel picker autentica", async () => {
    mockClient.me.mockRejectedValueOnce(new Error("no session")); // loadSession inicial
    mockClient.me.mockResolvedValueOnce(pickerUser); // me após login
    const { captured, unmount } = renderProvider();
    await flush();
    await act(async () => {
      await captured.current!.login("s@x.com", "pw");
    });
    expect(mockClient.login).toHaveBeenCalledWith({ email: "s@x.com", password: "pw" });
    expect(captured.current?.user?.name).toBe("Sep");
    unmount();
  });
