...
describe("usePushRegistration", () => {
  ...
  it("tap na notificação navega p/ data.route", async () => {
    ...
    listener({
      notification: { request: { content: { data: { route: "/task/t1" } } } },
    });
    expect(mockRouterPush).toHaveBeenCalledWith("/task/t1");
  });
...
describe("routeFromResponse", () => {
  ...
  it("retorna a rota quando presente", () => {
    expect(routeFromResponse(make({ route: "/task/9" }))).toBe("/task/9");
  });
