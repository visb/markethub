...
describe("usePushRegistration", () => {
  it("registra o token no backend ao autenticar", async () => {
    const api = makeApi();
    mockUseAuth.mockReturnValue({ user: { id: "u1" }, api });

    renderHook();
    await flush();

    expect(mockGetToken).toHaveBeenCalled();
    expect(api.registerDeviceToken).toHaveBeenCalledWith("ExponentPushToken[abc]", "ios");
  });
  ...
  it("revoga o token ao deslogar (transição user→null)", async () => {
    ...
    expect(api.unregisterDeviceToken).toHaveBeenCalledWith("ExponentPushToken[abc]");
  });
  ...
  it("tap na notificação navega p/ data.route", async () => {
    ...
    listener({
      notification: { request: { content: { data: { route: "/track/o1" } } } },
    });
    expect(mockRouterPush).toHaveBeenCalledWith("/track/o1");
  });
...
describe("routeFromResponse", () => {
  ...
  it("retorna a rota quando presente", () => {
    expect(routeFromResponse(make({ route: "/delivery/1" }))).toBe("/delivery/1");
  });
