 ...
 * - Tap na notificação: deep-link p/ `data.route` (ex.: `/track/<orderId>`).
...
async function register(api: ApiClient, tokenRef: { current: string | null }): Promise<void> {
  try {
    const token = await acquireToken();
    if (!token) return;
    tokenRef.current = token;
    await api.registerDeviceToken(token, currentPlatform());
  } catch {
    // best-effort
  }
}
...
export function usePushRegistration(): void {
  ...
  useEffect(() => {
    ...
    if (uid && uid !== prev) {
      void register(api, tokenRef);
    } else if (!uid && prev) {
      void revoke(api, tokenRef);
    }
  }, [user, api]);
