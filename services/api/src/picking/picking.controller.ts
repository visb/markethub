...
export class PickingController {
  ...
  /** Handoff: marca a tarefa (ensacolada) como pronta para coleta — gera o pickupCode (S3.6). */
  @Post(":id/ready")
  ...
  /**
   * Liberação de coleta (SF.1): a loja/picker digita o pickupCode apresentado
   * pelo entregador para liberar o pedido → OrderGroup on_the_way.
   */
  @Post(":id/release-pickup")
  releasePickup(
    @CurrentUser() _user: AuthUser,
    @Param("id") id: string,
    @Body() dto: ReleasePickupDto,
  ) {
    return this.handoff.confirmPickup(id, dto.pickupCode);
  }
}
