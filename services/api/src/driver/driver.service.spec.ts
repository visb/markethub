...
describe("DriverService.confirmDelivery", () => {
  it("recusa entrega que não é do entregador", async () => {
    ...
    const svc = new DriverService(prisma, { confirmDelivered: jest.fn() } as never, { emit: jest.fn() } as never);
  ...
  it("é idempotente quando já entregue", async () => {
    ...
    const svc = new DriverService(prisma, { confirmDelivered } as never, { emit: jest.fn() } as never);
