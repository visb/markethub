...
### Gate de cobertura (rígido)

- **Piso: 80% linhas em TODO workspace**, declarado no config de cada um (jest `coverageThreshold.global` / vitest `coverage.thresholds`, all-files cravado via `collectCoverageFrom`/`coverage.include`). Threshold violado = job vermelho.
- **Ratchet: piso só sobe.** Nunca baixar sem justificativa explícita no PR; workspaces acima de 80 mantêm o piso mais alto que já têm.
- **Código novo: diff ≥ 90%.** Em PR, `pnpm diff-coverage` exige linhas novas/alteradas ≥ 90% cobertas — arquivo novo sem teste reprova mesmo com o agregado ok.
- **Sem `--passWithNoTests`** em workspace que deve ter teste. `perFile` é decisão por workspace (declarada no config).

> Detalhe, racional do `perFile` por workspace e pendências manuais: **`docs/coverage.md`**.
